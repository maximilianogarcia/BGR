function MixViewModel() {
	   ...
	   self.getPaquetesByProducto = function(data){
		   ...
		   if( data.id in self.paquetesListCache ){
			  ...
			  $.postJSON(BASE_REST_URL+"/paquete/getPaquetesDisponiblesPorProducto",JSON.stringify(data.id)).
				 done(self.addToProductMap).
				 fail(self.alertar);
		   }
	   }	   
	   
	   self.addToProductMap = function(data){
		   self.paquetesListCache[data.id] = data.data;
	   ...
	   self.validarPaqueteProducto = function(paquete_id,producto_id){
		  ...
		  if ( producto_id in self.paquetesListCache){
			  exist =  ko.utils.arrayFirst(self.paquetesListCache[producto_id], function(item) {
	   ...
	   self.loadForMixProduct = function(){
		   ...
		   self.utils.getAll(self.allUnidades, "/unidadDeMedida/getAll");
		   ...
		   for(var i = 0 ;i < self.paquetesPorProductoSelected().length; i++){		
	  	  ...
	  	  $.postJSON(BASE_REST_URL+"/unidadDeMedida/getTotalByPaquetes",JSON.stringify(paquetes)).
	  	  done(function(result) { 
	  		self.allUnidadesTotalizadas(result);
	 	  });
	   }
	   ...
	   self.guardar = function(){
		  ...
		  $.postJSON(BASE_REST_URL+"/mix/saveMix",JSON.stringify(mix)).
	  	  done(function(result) { 
	  		   if(result == null){
        		   alert("las cantidades son incompatibles");
        	   }else{
        	   alert("se ha guardado el mix exitosamente");
        	   location.href = "mixGrid.html";
           		}
	 	  }).fail(function(result) {alert(result.responseText);});
