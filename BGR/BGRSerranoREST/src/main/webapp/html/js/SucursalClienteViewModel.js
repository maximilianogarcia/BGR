function SucursalViewModel() {
	...
	self.eoi = ko.mapping.fromJS(new Cliente());
	...
	self.init = function() {
		...
		$.getJSON(BASE_REST_URL+"/sucursal/listByCliente/"+self.eoi.id(), function(data){  
	...
	self.display= function(cli){
		ko.mapping.fromJS(cli, self.eoi);
		self.init();
		self.showable(true);
	}
	...
	self.backToCliente= function(){
		self.formVisible(false);
		showClienteOpen(self.eoi);
	}
